...
import vn.edu.hcmut.emrre.feature.ConceptFeatureVn;
...
public class ConceptExtractor {
	...
	public static void setConcepts(List<Concept> concepts) {
		ConceptExtractor.concepts = concepts;
	}

	private void getConceptData() {
		if (ConceptExtractor.concepts == null) {
			ReadFile read = new ReadFile();
			read.setFolder("vn/concept");
			ConceptExtractor.concepts = read.getAllConcept(0);
		}
	}
	...
	private List<Concept> getDoclineConcepts(String fileName, int lineIdx) {
		...
		for (Concept concept : ConceptExtractor.concepts) {
	...
	private void generateBIO() {
		...
		if (ConceptExtractor.doclines == null) {
			return;
		}
		BIOs = new ArrayList<int[]>();
		for (idx = 0; idx < ConceptExtractor.doclines.size(); idx++) {
			doc = ConceptExtractor.doclines.get(idx);
	...
	public void crossValidation(int numFold) throws IOException, InvalidInputDataException {
		...
		SentenceDAO senDao = SentenceDAO.Instance;
		Sentence sentence;
